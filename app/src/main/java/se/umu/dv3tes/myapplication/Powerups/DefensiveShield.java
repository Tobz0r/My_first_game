 ...
 */
public class DefensiveShield extends GameObject implements Powerup {
    ...
    public DefensiveShield(Player player, Handler handler,Bitmap image,int x){
        setVelX(0);
        setVelY(5);
        setY(0);
    ...
    public void tick() {
        ...
        for(int i=0; i < projectiles.size();i++){
            ...
            if (getBounds().intersect(temp.getBounds())){
                if(!temp.isHostile()){
                    player.powerUp(Powerups.DEFENSE);
